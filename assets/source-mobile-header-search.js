(() => {
  ...
  function initHeaderSearch(header) {
    ...
    const defaultSearchData = {
      products: [
        { title: 'Little Sunflower Print', url: '/products/little-sunflower-print' },
        { title: 'Soft Heart Print', url: '/products/soft-heart-print' },
        { title: 'I Love You Print', url: '/products/i-love-you-print' },
        { title: 'Curious Sloth Print', url: '/products/curious-sloth-print' },
        { title: 'Robot King Print', url: '/products/robot-king-print' },
        { title: 'No Rules Print', url: '/products/no-rules-print' },
      ],
      collections: [
        { title: "Children's Hand-Drawn Art", url: '/collections/儿童手绘' },
        { title: 'Bestseller', url: '/collections/bestseller' },
        { title: 'Personalisierbar', url: '/collections/personalisierbar' },
        { title: 'Leinwand', url: '/collections/leinwand' },
        { title: 'Acrylglas', url: '/collections/acrylglas' },
        { title: 'Poster', url: '/collections/poster' },
        { title: 'Abstrakt', url: '/collections/abstrakt' },
      ],
    };
    let localSearchData = defaultSearchData;

    if (!panel || !input) return;

    try {
      const embeddedData = JSON.parse(document.getElementById('SourceMobileSearchData')?.textContent || '{}');
      localSearchData = {
        products: embeddedData.products?.length ? embeddedData.products : defaultSearchData.products,
        collections: embeddedData.collections?.length ? embeddedData.collections : defaultSearchData.collections,
      };
    } catch (error) {
      localSearchData = defaultSearchData;
    }
